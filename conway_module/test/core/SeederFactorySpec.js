...
const {
	...
	SeederModels,
	Seeder,
} = require('./../../lib/core/SeederFactory.js');
...
describe('Seeder Factory', function () {
	...
	it('should throw an error on unknown models', function () {
		...
		expect(() => SeederFactory.build(badModelName)).to.throw(
			`Unknown seeder model name: ${badModelName}`
		);
	});
	...
	describe('StaticCellsSeeder', function () {
		it('should just return the cells provided', function () {
			...
			expect(model.seed()).to.have.deep.ordered.members([
				new Cell(1, 2, 5),
				new Cell(33, 1, 1),
			]);
		});
